// ==UserScript==
// @name         Auto-Matrix
// @namespace    https://matrix.sysu.edu.cn/
// @version      1.7
// @description  自动调用 LLM API 完成 Matrix 平台上的编程题与选择题
// @author       hyw
// @match        https://matrix.sysu.edu.cn/*
// @grant        GM_addStyle
...
5. 不要输出多余的解释，只输出代码块。`,
    ...
    async function applyCodeBlocks(codeBlocks, files) {
        ...
        for (const block of codeBlocks) {
            ...
            // 先切换到目标文件的 tab，确保 Monaco 焦点在正确的编辑器上
            const tabs = getTabsFromDOM();
        ...
        // 终极 fallback：如果完全没写入成功，尝试用第一个可编辑文件
...
3. 不要输出任何解释，只输出答案。`;
    ...
    function extractQuestionFromCard(card, idx) {
        ...
        // 题干：card-body 内 radio-group 之前的内容
        let stem = '';
    ...
    async function choiceMainLoop(setStatus, isRunning) {
        ...
        let globalQuestionIndex = 0; // 跨页累计题号
